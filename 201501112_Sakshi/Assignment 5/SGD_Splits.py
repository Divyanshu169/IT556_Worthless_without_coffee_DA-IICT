...
for i in range(2,15):
    ...
    data = surprise.Dataset.load_from_file('/Users/keyadesai/Desktop/Recommendation Engine/ratings_normalised.csv', reader=reader)
    #data.split(5)
    ...
    param_grid = {'n_epochs': [10], 'lr_all': [0.005],
                  'reg_all': [0.02]}
    start=time.clock()
    gs = GridSearchCV(SVD, param_grid, measures=['rmse', 'mae'], cv=i)
    gs.fit(data)
...
plt.xlabel('Number of splits')
plt.ylabel(('Time'))
plt.title('Splits v/s Time' )
plt.show()

plt.plot(x_plot,mem_plot,'ro')
plt.xlabel('Number of splits')
plt.ylabel(('Memory'))
plt.title('Splits v/s Memory' )
plt.show()
